...
def build_spy_bh(spy_df: pd.DataFrame, strategy_dates: list) -> dict:
    ...
    spy_window = spy_df[(spy_df.index >= pd.Timestamp(start)) & (spy_df.index <= pd.Timestamp(end))].copy()
...
def plot_equity(baseline: dict, spy_bh: dict):
    ...
    def _plot_window(ax, x_start, x_end, title_str, show_ylabel, show_legend):
        ...
        if ds_s:
            ax.plot(ds_s, es_s, label="Overnight Strategy (Baseline)", color=COLOR_STRAT, linewidth=1.8, linestyle="-")
        if ds_b:
            ax.plot(ds_b, es_b, label="SPY Buy & Hold", color=COLOR_SPY, linewidth=1.2, linestyle="--")
    ...
    _plot_window(ax1a, start_date, end_date, "Panel A — Full Period (Mar 2024 – Mar 2026)", show_ylabel=True, show_legend=True)
    ...
    _plot_window(ax1b, start_6m, end_date, "Panel A — Last 6 Months (Zoom)", show_ylabel=False, show_legend=False)
    ...
    fig.suptitle("Overnight Effect — Baseline Strategy vs. SPY Buy & Hold", fontsize=13, fontweight="bold")

    plt.savefig("baseline_fig1_equity.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
def plot_drawdown(baseline: dict, spy_bh: dict):
    ...
    ax.fill_between(baseline["dates"], dd_s, 0, alpha=0.25, color=COLOR_STRAT_DD, zorder=1)
    ax.fill_between(spy_bh["dates"], dd_b, 0, alpha=0.18, color=COLOR_SPY_DD, zorder=0)
    ax.plot(baseline["dates"], dd_s, color=COLOR_STRAT_DD, linewidth=1.6, label="Overnight Strategy (Baseline)")
    ax.plot(spy_bh["dates"], dd_b, color=COLOR_SPY_DD, linewidth=1.2, linestyle="--", label="SPY Buy & Hold")
    ...
    plt.savefig("baseline_fig2_drawdown.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
def plot_table(m_baseline: dict, m_spy: dict):
    ...
    plt.savefig("baseline_fig3_table.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
    print("Saved: baseline_fig3_table.png")
    plt.close()


def plot_comparison(baseline: dict, spy_bh: dict, m_baseline: dict, m_spy: dict):
...
if __name__ == "__main__":
    ...
    m_baseline = compute_metrics(baseline["equity"], baseline["rets"], "Overnight Strategy (Baseline)", baseline["dates"])
    m_spy = compute_metrics(spy_bh["equity"],   spy_rets, "SPY Buy & Hold", spy_bh["dates"])
