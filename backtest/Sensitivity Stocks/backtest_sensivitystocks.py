...
def build_clean_data(raw_data: pd.DataFrame, tickers: list, momentum_window: int) -> dict:
    clean = {}
    for t in tickers:
        try:
            df = raw_data[t].copy()
...
def run_strategy(clean_data: dict, top_n: int, momentum_window: int) -> dict:
...
def _legend_elements():
    ...
    return [
        ...
        Line2D([0], [0], color="#888888", linewidth=1.5, linestyle=LINESTYLES[63], label="63d momentum"),
        Line2D([0], [0], color="#888888", linewidth=2.0, linestyle=LINESTYLES[126], label="126d momentum  ★ baseline"),
    ]
...
def plot_sens_equity(results: dict):
    ...
    ax.set_title("Panel A — Equity Curves (Mar 2024 – Mar 2026)", loc="left", fontsize=10, fontweight="bold", pad=6)
    ax.grid(True, axis="y")
    ...
    fig.suptitle("Sensitivity Analysis — Portfolio Size × Momentum Window", fontsize=13, fontweight="bold")
    
    plt.savefig("sensitivity_fig1_equity.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
def plot_sens_drawdown(results: dict):
    ...
    for (top_n, mw), res in sorted(results.items()):
        ...
        ax.plot(res["dates"], dd, color=COLORS[top_n], linestyle=LINESTYLES[mw], linewidth=LINEWIDTHS[mw] + (0.6 if is_baseline else 0),
                alpha=0.95 if is_baseline else 0.70,
                zorder=10 if is_baseline else 5)

    dd_base = _calc_dd(results[(10, 126)]["equity"])
    ax.fill_between(results[(10, 126)]["dates"], dd_base, 0, alpha=0.12, color=COLORS[10], zorder=0)
    ...
    plt.savefig("sensitivity_fig2_drawdown.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
def plot_sens_table(metrics: dict):
    ...
    col_headers = ["Scenario", "Final Value", "Ann. Return", "Volatility", "Sharpe", "Sortino", "Max DD", "Win Rate"]
    rows = []
    ...
    for (r, c), cell in tbl.get_celld().items():
        ...
        if r == 0:
            ...
            cell.set_text_props(color="white", fontweight="bold", ha="left" if c == 0 else "center", fontsize=9)
            cell.PAD = 0.05
        else:
    ...
    ax.set_title("Panel C — Performance Summary by Configuration", loc="left", fontsize=10, fontweight="bold", pad=8)

    plt.savefig("sensitivity_fig3_table.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
if __name__ == "__main__":
    ...
    for top_n, mw in itertools.product(TOP_N_VALUES, MOMENTUM_WINDOWS):
        ...
        m = compute_metrics(clipped["equity"], clipped["rets"], label, clipped["dates"])
