"""
Backtest: Day & Night Strategy — Baseline vs VIX Volatility Filter
Compares the unfiltered baseline (TOP-10 / 126d) against the same strategy
with a VIX filter: only enters positions when VIX is below a threshold,
avoiding high fear / high volatility regimes.
...
TOP_N            = 10
MOMENTUM_WINDOW  = 126
VIX_THRESHOLD    = 25        # suspend trading when VIX >= 25
RISK_FREE_RATE   = 0.0
...
def run_strategy(clean_data: dict, vix_df: pd.DataFrame,
                 use_vix: bool) -> dict:
    """
    use_vix=False → baseline (no filter)
    use_vix=True  → only trade when VIX < VIX_THRESHOLD
...
def plot_equity(baseline: dict, vix: dict):
    ...
    def _panel(ax, x0, x1, title, show_ylabel, show_legend):
        ...
        if ds_v:
            ax.plot(ds_v, es_v, label=f"VIX Filter (< {VIX_THRESHOLD})",
                    color=COLOR_VIX, linewidth=1.8, linestyle="--")
    ...
    fig.suptitle("Overnight Effect — Baseline vs. VIX Volatility Filter",
                 fontsize=13, fontweight="bold")
    
    plt.savefig("vix_fig1_equity.png", dpi=200, bbox_inches="tight",
...
def plot_drawdown(baseline: dict, vix: dict):
    ...
    ax.plot(vix["dates"],      dd_v, color=COLOR_DD_VIX,  linewidth=1.6,
            linestyle="--", label=f"VIX Filter (< {VIX_THRESHOLD})")
    ...
    plt.savefig("vix_fig2_drawdown.png", dpi=200, bbox_inches="tight",
                facecolor="white", edgecolor="none")
...
def plot_table(m_b: dict, m_v: dict):
    ...
    plt.savefig("vix_fig3_table.png", dpi=200, bbox_inches="tight",
                facecolor="white", edgecolor="none")
...
if __name__ == "__main__":
    ...
    m_vix      = compute_metrics(vix["equity"], vix["rets"],
                                 f"VIX Filter (< {VIX_THRESHOLD})", vix["dates"])
