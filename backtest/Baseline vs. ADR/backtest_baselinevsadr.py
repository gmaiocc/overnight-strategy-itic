"""
Backtest: Day & Night Strategy — Baseline vs ADR Filter
Compares the unfiltered baseline (TOP-10 / 126d) against the same strategy
with an ADR filter: excludes stocks whose Average Daily Range over the last
N days exceeds a threshold, avoiding high intraday volatility candidates.

ADR = mean((High - Low) / Close) over ADR_WINDOW days
Only stocks with ADR >= ADR_THRESHOLD are eligible for selection.
...
TOP_N            = 10
MOMENTUM_WINDOW  = 126
ADR_WINDOW       = 20          # days to compute average daily range
ADR_THRESHOLD    = 0.05        # only include stocks with ADR > 5%
RISK_FREE_RATE   = 0.0
...
def build_clean_data(raw_data: pd.DataFrame, tickers: list) -> dict:
    ...
    for t in tickers:
        try:
            ...
            # ADR: average daily range over ADR_WINDOW days
            df["DailyRange"] = (df["High"] - df["Low"]) / df["Close"]
            df["ADR"] = df["DailyRange"].rolling(ADR_WINDOW).mean()
...
def run_strategy(clean_data: dict, use_adr: bool) -> dict:
    """
    use_adr=False → baseline (no filter)
    use_adr=True  → only include stocks with ADR >= ADR_THRESHOLD on that day
    """
    ...
    for i in range(MOMENTUM_WINDOW + 5, len(all_dates) - 1):
        ...
        for t, df in clean_data.items():
            ...
            # ── ADR filter: only keep stocks with high intraday range ──────────
            if use_adr:
                if pd.isna(row["ADR"]) or row["ADR"] < ADR_THRESHOLD:
                    continue
...
COLOR_ADR      = "#D67728"   # orange — ADR filter
COLOR_DD_BASE  = "#D62728"   # red    — baseline drawdown
COLOR_DD_ADR   = "#D67728"   # orange — ADR drawdown
...
def plot_equity(baseline: dict, adr: dict):
    _rcparams()
    ...
    def _panel(ax, x0, x1, title, show_ylabel, show_legend):
        ...
        ds_a, es_a = clip(adr,      x0, x1)
        if ds_b:
            ax.plot(ds_b, es_b, label="Baseline (no filter)",
                    color=COLOR_BASELINE, linewidth=1.8)
        if ds_a:
            ax.plot(ds_a, es_a, label=f"ADR Filter (≥ {ADR_THRESHOLD*100:.0f}%)",
                    color=COLOR_ADR, linewidth=1.8, linestyle="--")
    ...
    full_vals = baseline["equity"] + adr["equity"]
    ...
    zoom_vals += [e for d, e in zip(adr["dates"],      adr["equity"])      if d >= start_6m]
    ...
    fig.suptitle("Overnight Effect — Baseline vs. ADR High-Volatility Filter",
                 fontsize=13, fontweight="bold")

    plt.savefig("adr_fig1_equity.png", dpi=200, bbox_inches="tight",
                facecolor="white", edgecolor="none")
    print("  Saved: adr_fig1_equity.png")
    plt.close()


def plot_drawdown(baseline: dict, adr: dict):
    _rcparams()
    start_date = baseline["dates"][0]
    end_date   = baseline["dates"][-1]
    ...
    ax.fill_between(adr["dates"],      dd_a, 0, alpha=0.20, color=COLOR_DD_ADR,  zorder=0)
    ax.plot(baseline["dates"], dd_b, color=COLOR_DD_BASE, linewidth=1.6,
            label="Baseline (no filter)")
    ax.plot(adr["dates"],      dd_a, color=COLOR_DD_ADR,  linewidth=1.6,
            linestyle="--", label=f"ADR Filter (≥ {ADR_THRESHOLD*100:.0f}%)")
    ...
    plt.savefig("adr_fig2_drawdown.png", dpi=200, bbox_inches="tight",
                facecolor="white", edgecolor="none")
    print("  Saved: adr_fig2_drawdown.png")
    plt.close()


def plot_table(m_b: dict, m_a: dict):
    _rcparams()

    col_headers = ["Metric", "Baseline", f"ADR Filter (≥ {ADR_THRESHOLD*100:.0f}%)"]
    rows = [
        ["Final Value",   f"${m_b['final_cap']:,.0f}",     f"${m_a['final_cap']:,.0f}"],
        ["Total Return",  f"{m_b['total_ret']*100:+.2f}%", f"{m_a['total_ret']*100:+.2f}%"],
        ["Ann. Return",   f"{m_b['ann_ret']*100:+.2f}%",   f"{m_a['ann_ret']*100:+.2f}%"],
        ["Volatility",    f"{m_b['vol']*100:.2f}%",        f"{m_a['vol']*100:.2f}%"],
        ["Sharpe Ratio",  f"{m_b['sharpe']:.2f}",          f"{m_a['sharpe']:.2f}"],
        ["Sortino Ratio", f"{m_b['sortino']:.2f}",         f"{m_a['sortino']:.2f}"],
        ["Max Drawdown",  f"{m_b['max_dd']*100:.2f}%",     f"{m_a['max_dd']*100:.2f}%"],
        ["Win Rate",      f"{m_b['win_rate']*100:.1f}%",   f"{m_a['win_rate']*100:.1f}%"],
    ]
    ...
    plt.savefig("adr_fig3_table.png", dpi=200, bbox_inches="tight",
                facecolor="white", edgecolor="none")
    print("  Saved: adr_fig3_table.png")
    plt.close()
...
if __name__ == "__main__":
    print("\n" + "="*62)
    print(f"  BACKTEST: Baseline vs ADR Filter (threshold = {ADR_THRESHOLD*100:.0f}%)")
    print("="*62)
    ...
    print("  → Baseline...")
    baseline_raw = run_strategy(clean_data, use_adr=False)
    baseline     = clip_to_eval(baseline_raw)

    print(f"  → ADR Filter (ADR ≤ {ADR_THRESHOLD*100:.0f}%)...")
    adr_raw = run_strategy(clean_data, use_adr=True)
    adr     = clip_to_eval(adr_raw)

    m_baseline = compute_metrics(baseline["equity"], baseline["rets"],
                                 "Baseline (no filter)", baseline["dates"])
    m_adr      = compute_metrics(adr["equity"], adr["rets"],
                                 f"ADR Filter (≥ {ADR_THRESHOLD*100:.0f}%)", adr["dates"])

    print_metrics(m_baseline)
    print_metrics(m_adr)

    print(f"\n{'='*62}")
    print(f"  Sharpe improvement:   {m_adr['sharpe'] - m_baseline['sharpe']:+.2f}")
    print(f"  Ann. return delta:    {(m_adr['ann_ret'] - m_baseline['ann_ret'])*100:+.2f}%")
    print(f"  Max DD improvement:   {(m_adr['max_dd'] - m_baseline['max_dd'])*100:+.2f}%")
    print(f"{'='*62}\n")

    plot_equity(baseline, adr)
    plot_drawdown(baseline, adr)
    plot_table(m_baseline, m_adr)