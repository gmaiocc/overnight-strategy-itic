...
def plot_equity(baseline: dict, adr: dict):
    ...
    def _panel(ax, x0, x1, title, show_ylabel, show_legend):
        ...
        if ds_a:
            ax.plot(ds_a, es_a, label=f"ADR Filter (≥ {ADR_THRESHOLD*100:.0f}%)", color=COLOR_ADR, linewidth=1.8, linestyle="--")
    ...
    fig.suptitle("Overnight Effect — Baseline vs. ADR High-Volatility Filter", fontsize=13, fontweight="bold")

    plt.savefig("adr_fig1_equity.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
def plot_drawdown(baseline: dict, adr: dict):
    ...
    ax.plot(adr["dates"], dd_a, color=COLOR_DD_ADR,  linewidth=1.6, linestyle="--", label=f"ADR Filter (≥ {ADR_THRESHOLD*100:.0f}%)")
    ...
    plt.savefig("adr_fig2_drawdown.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
def plot_table(m_b: dict, m_a: dict):
    ...
    plt.savefig("adr_fig3_table.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
if __name__ == "__main__":
    ...
    m_adr = compute_metrics(adr["equity"], adr["rets"], f"ADR Filter (≥ {ADR_THRESHOLD*100:.0f}%)", adr["dates"])
