...
def fetch_spy_ma50(start: str, end: str) -> pd.DataFrame:
    print("Downloading SPY (MA50 & Buy/Hold)...")
...
def run_spy_bh(spy_df: pd.DataFrame, target_dates: list) -> dict:
    """Simula um Buy & Hold do SPY nas mesmas datas da estratégia."""
    equity, rets = [], []
    ...
    return {"dates": target_dates, "equity": equity, "rets": rets}


def run_longshort(clean_data: dict, spy_df: pd.DataFrame) -> dict:
    """
    Regime-based:
      SPY > MA50  -> LONG  TOP-10  by momentum  (overnight premium)
      SPY <= MA50 -> SHORT BOTTOM-10 by momentum (overnight discount)
    """
...
def plot_equity(spy_bh, ls):
    _rcparams()
    start_date = spy_bh["dates"][0]
    end_date = spy_bh["dates"][-1]
    ...
    def _panel(ax, x0, x1, title, show_ylabel, show_legend):
        ds_b, es_b = clip(spy_bh, x0, x1)
        ds_l, es_l = clip(ls, x0, x1)
        if ds_b:
            ax.plot(ds_b, es_b, label="Buy & Hold SPY", color=COLOR_SPY, linewidth=1.8)
        if ds_l:
    ...
    full_vals = [v for v in spy_bh["equity"] + ls["equity"] if not np.isnan(v)]
    ...
    fig.suptitle("Overnight Effect — Buy & Hold SPY vs. MA50 Long/Short Regime", fontsize=13, fontweight="bold")
    plt.savefig("ls_fig1_equity.png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
    print("Saved: ls_fig1_equity.png")
    plt.close()


def plot_drawdown(spy_bh, ls):
    _rcparams()
    start_date = spy_bh["dates"][0]
    end_date = spy_bh["dates"][-1]
    ...
    ax.fill_between(spy_bh["dates"], dd_b, 0, alpha=0.20, color=COLOR_DD_SPY, zorder=1)
    ax.fill_between(ls["dates"], dd_l, 0, alpha=0.20, color=COLOR_DD_LS, zorder=0)
    
    ax.plot(spy_bh["dates"], dd_b, color=COLOR_DD_SPY, linewidth=1.6, label="Buy & Hold SPY")
...
def plot_table(m_spy, m_ls):
    _rcparams()
    col_headers = ["Metric", "Buy & Hold SPY", "L/S Regime (MA50)"]
    rows = [
        ["Final Value",   f"${m_spy['final_cap']:,.0f}",     f"${m_ls['final_cap']:,.0f}"],
        ["Total Return",  f"{m_spy['total_ret']*100:+.2f}%", f"{m_ls['total_ret']*100:+.2f}%"],
        ["Ann. Return",   f"{m_spy['ann_ret']*100:+.2f}%",   f"{m_ls['ann_ret']*100:+.2f}%"],
        ["Volatility",    f"{m_spy['vol']*100:.2f}%",        f"{m_ls['vol']*100:.2f}%"],
        ["Sharpe Ratio",  f"{m_spy['sharpe']:.2f}",          f"{m_ls['sharpe']:.2f}"],
        ["Sortino Ratio", f"{m_spy['sortino']:.2f}",         f"{m_ls['sortino']:.2f}"],
        ["Max Drawdown",  f"{m_spy['max_dd']*100:.2f}%",     f"{m_ls['max_dd']*100:.2f}%"],
        ["Win Rate",      "N/A",                             f"{m_ls['win_rate']*100:.1f}%"],
    ]
...
if __name__ == "__main__":
    print("\n" + "="*62)
    print("BACKTEST: Buy & Hold SPY vs MA50 Long/Short Regime")
    print("="*62)
    ...
    print("→ Buy & Hold SPY (matching dates)...")
    spy_bh_raw = run_spy_bh(spy_df, ls_raw["dates"])
    spy_bh = clip_to_eval(spy_bh_raw)

    m_spy = compute_metrics(spy_bh["equity"], spy_bh["rets"], "Buy & Hold SPY", spy_bh["dates"])
    ...
    print(f"\n{'='*62}")
    print(f"Sharpe improvement: {m_ls['sharpe'] - m_spy['sharpe']:+.2f}")
    print(f"Ann. return delta: {(m_ls['ann_ret'] - m_spy['ann_ret'])*100:+.2f}%")
    print(f"Max DD improvement: {(m_ls['max_dd'] - m_spy['max_dd'])*100:+.2f}%")
    print(f"{'='*62}\n")

    plot_equity(spy_bh, ls)
    plot_drawdown(spy_bh, ls)
    plot_table(m_spy, m_ls)